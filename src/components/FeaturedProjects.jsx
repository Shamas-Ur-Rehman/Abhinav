import React, { useState } from 'react';
import { useRef } from 'react';
import Navbar from './Navbar';

const ProjectCard = ({ project, isEven }) => (
  <div className={`relative mb-32 ${isEven ? 'text-right' : 'text-left'}`}>
    <div className="relative z-10 w-full md:w-7/12 mb-8">
      <img src={project.image} alt={project.title} className="w-full rounded-lg shadow-xl" />
    </div>
    {/* // text Card */}
    <div className={`absolute top-1/2 ${isEven ? 'left-0' : 'right-0'} w-full md:w-7/12 shadow-[#032388] -mt-20 p-8 bg-white rounded-lg shadow-xl z-20`}>
      <h4 className="text-[#032388] text-sm mb-2">Featured Project</h4>
      <h3 className="text-2xl font-bold text-black mb-4">{project.title}</h3>
      <p className="text-gray-400 mb-4">{project.description}</p>
      <div className="flex flex-wrap mb-4 justify-end">
        {project.technologies.map((tech, index) => (
          <span key={index} className="text-gray-400 text-sm ml-4 mb-2">{tech}</span>
        ))}
      </div>
      {/* <div className="flex justify-end">
        {project.github && (
          <a href={project.github} target="_blank" rel="noopener noreferrer" className="ml-4">
            <GitHub className="w-6 h-6 text-gray-400 hover:text-[#032388]" />
          </a>
        )}
      </div> */}
    </div>
  </div>
);

const FeaturedProjects = () => {
    const featureProjectRef = useRef(null);


  const [projects] = useState([
    {
      title: "Digital Bridges",
      description: "Throughout high school, I was often asked, 'What do you want to pursue?'—a question I still don’t have a clear answer to. I was fascinated by how others seemed to find their passion so early, often without fully exploring the world of possibilities. That’s why I created What is Your Passion?—a database of interviews with people from various fields, offering aspiring minds a deeper understanding of how individuals discovered their paths. Additionally, this initiative bridges the gap between generations by teaching digital literacy to the elderly in old age homes, inspired by my own experience of disconnect with my grandmother",
      image: "data:image/jpeg;base64,", // Replace with actual image path
      technologies: [],
    },
    
    
  ]);

  return (
    <>
    <Navbar featureProjectRef={featureProjectRef} />
    
    <div ref={featureProjectRef} className="bg-white min-h-screen text-black font-sans p-10" id='work'>
      <div className="max-w-4xl mx-auto">
        <h2 className="text-3xl font-bold mb-12">
          <span className="text-[#032388] mr-2">03.</span>
          Featured Projects
        </h2>
        
        {projects.map((project, index) => (
          <ProjectCard key={index} project={project} isEven={index % 2 !== 0} />
        ))}
      </div>
      
    </div>
    </>
  );
};

export default FeaturedProjects;